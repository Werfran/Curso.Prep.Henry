...
function dePalabrasAFrase(palabras) {
  ...
  for (let index = 0; index < palabras.length; index++) {
    concat += palabras[index];
    if (index != palabras.length - 1) concat += " ";
  }
  return concat;
}
...
function diaDeLaSemana(numeroDeDia) {
  ...
  //Escribe tu código aquí
  if (numeroDeDia % 7 >= 2) return "Es dia Laboral";
...
function mesesDelAño(array) {
  ...
  // Tu código:
  const meseseBuscados = ["Enero", "Marzo", "Noviembre"];
  var mesesEncontrados = [];
  array.forEach((mes) => {
    if (meseseBuscados.includes(mes)) mesesEncontrados.push(mes);
  });
...
function mayorACien(array) {
  ...
  array.forEach((valor) => {
    if (valor > 100) valoresEncontrados.push(valor);
  });
...
function breakStatement(numero) {
  ...
  for (let index = 0; index < 10; index++) {
    ...
    if (suma == index + 1) return "Se interrumpió la ejecución";
  }
  return numeros;
}
